...
- **Загрузить CRM** — ручная выгрузка Hollihop в `raw_records` (лиды, ученики и финансы) и векторизация в `doc_chunks`;
...
остальными, помечаются в `raw_records` флагом `is_financial` и векторизуются в
`doc_chunks`. Такие фрагменты подмешиваются в ответы только администраторам —
фильтр `search_chunks(include_financial=...)` скрывает их для не-админов.
