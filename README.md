...
точечные refresh-запросы в личке вроде `обнови CRM`, `обнови платежи` и
`обнови группы`; в обычном режиме бот отвечает из локально синхронизированных данных.
...
секции `GetEdUnits`/`GetEdUnitStudents`) выгружаются вместе с остальными,
помечаются флагом `is_financial` и векторизуются в `doc_chunks`. Такие фрагменты
подмешиваются в ответы только администраторам — фильтр
`search_chunks(include_financial=...)` скрывает их для не-админов.
